...
function SQL_Injection($get_Str) {
	$out = preg_replace("(select|union|insert|update|delete|drop|\"|\'|#|--|\/\*|\*\/|\\\|\;)","", $get_Str);
	$out = preg_replace("/[\"'%()$!&?: #]/","",  @mysql_real_escape_string($out));
	return $out;
}
...
class SE {
	...
	*/
	...
	public static function _clean_data($str) {
		...
		$str = SE::remove_invisible_characters($str, TRUE);
	...
	public static function _clear($array)
	{
		...
		if (is_array($array))
		{
			...
			foreach($array as $k=>$v) {
				...
				if (!is_array($v)) {
					$out[SE::_clean_key($k)] = addslashes(SE::_clean_data(SQL_Injection($array[$k])));
				} else {

					foreach($v as $a=>$b)
					{
						$out[SE::_clean_key($k)][] = addslashes(SE::_clean_data(SQL_Injection($v[$a])));
					}
